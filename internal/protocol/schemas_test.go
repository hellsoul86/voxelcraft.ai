...
func TestSchemas_ValidateSamples(t *testing.T) {
	...
	_ = json.Unmarshal([]byte(`{
	  ...
	  "self":{"pos":[0,10,0],"yaw":0,"hp":20,"hunger":20,"stamina":1.0,"status":["NONE"]},
	  "inventory":[],
	  ...
	  "voxels":{"center":[0,10,0],"radius":7,"encoding":"RLE","data":"AA=="},
	  "entities":[],
	  "events":[],
	  "tasks":[]
	}`), &obs)
	...
	_ = json.Unmarshal([]byte(`{
	  ...
	  "tasks":[{"id":"K1","type":"MOVE_TO","target":[1,10,1],"tolerance":1.2}],
	  "cancel":[]
	}`), &act)
