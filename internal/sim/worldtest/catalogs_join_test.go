...
import (
	...
	sessioncatalogspkg "voxelcraft.ai/internal/sim/world/feature/session/catalogs"
)
...
func TestJoin_CatalogsIncludeFullSet(t *testing.T) {
	...
	w, err := world.New(world.WorldConfig{ID: "test", Seed: 1}, cats)
	if err != nil {
		t.Fatalf("world.New: %v", err)
	}

	resp := make(chan world.JoinResponse, 1)
	_, _ = w.StepOnce([]world.JoinRequest{{
		Name:        "bot",
		DeltaVoxels: false,
	...
	// Validate expected digests (client cache keys).
	if jr.Catalogs[0].Digest != cats.Blocks.PaletteDigest {
		t.Fatalf("block_palette digest mismatch")
	}
	if jr.Catalogs[1].Digest != cats.Items.PaletteDigest {
		t.Fatalf("item_palette digest mismatch")
	}
	if jr.Catalogs[3].Digest != cats.Recipes.Digest {
		t.Fatalf("recipes digest mismatch")
	}
	if jr.Catalogs[4].Digest != cats.Blueprints.Digest {
		t.Fatalf("blueprints digest mismatch")
	}
	if jr.Catalogs[5].Digest != cats.Laws.Digest {
		t.Fatalf("law_templates digest mismatch")
	}
	if jr.Catalogs[6].Digest != cats.Events.Digest {
	...
	if tc, ok := jr.Catalogs[2].Data.(sessioncatalogspkg.TuningCatalog); !ok || tc.SnapshotEveryTicks <= 0 {
	...
	if lc, ok := jr.Catalogs[5].Data.(sessioncatalogspkg.LawTemplatesCatalogData); !ok || len(lc.Templates) == 0 {
