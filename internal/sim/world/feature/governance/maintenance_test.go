...
func TestEffectiveMaintenanceCost(t *testing.T) {
	got := EffectiveMaintenanceCost(nil)
...
func TestNextMaintenanceDue(t *testing.T) {
	if got := NextMaintenanceDue(100, 0, 6000); got != 6100 {
		t.Fatalf("expected 6100, got %d", got)
	}
	if got := NextMaintenanceDue(100, 7000, 6000); got != 13000 {
		t.Fatalf("expected 13000, got %d", got)
	}
}

func TestNextMaintenanceStage(t *testing.T) {
	if got := NextMaintenanceStage(2, true); got != 0 {
		t.Fatalf("expected reset to 0, got %d", got)
	}
	if got := NextMaintenanceStage(0, false); got != 1 {
		t.Fatalf("expected 1, got %d", got)
	}
	if got := NextMaintenanceStage(2, false); got != 2 {
