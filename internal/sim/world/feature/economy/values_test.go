...
func TestTradeValue_DeterministicAndPositive(t *testing.T) {
...
func TestTradeMutualBenefit(t *testing.T) {
	fairOffer := TradeValue(map[string]int{"PLANK": 5}, ItemTradeValue)        // 5
	fairRequest := TradeValue(map[string]int{"IRON_INGOT": 1}, ItemTradeValue) // 5
	if !TradeMutualBenefit(fairOffer, fairRequest) {
		t.Fatalf("expected fair trade to be mutual benefit")
	}

	unfairOffer := TradeValue(map[string]int{"PLANK": 1}, ItemTradeValue)           // 1
	unfairRequest := TradeValue(map[string]int{"CRYSTAL_SHARD": 1}, ItemTradeValue) // 20
